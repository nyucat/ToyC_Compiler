...
int runAssemblyCase(const std::string& name, const std::string& source, bool optimize) {
    ...
    try {
        ...
        failures += expectContains(text, "sw a0, 0(sp)", name + ".store-call-result-in-frame");
        failures += expectContains(text, "lw t0, 0(sp)", name + ".load-call-result-in-frame");
        failures += expectNotContains(text, "-4(sp)", name + ".no-negative-stack-slot");
    } catch (const std::exception& ex) {
        std::cout << "[FAIL] " << name << " exception: " << ex.what() << '\n';
        ++failures;
    }
...
int main() {
    ...
    failures += runPipelineCase("pipeline-opt-fold",
                                "int main() { return 2 + 3; }",
                                true);
    failures += runAssemblyCase("backend-global-call-saved-reg",
                                "int a = 1;\n"
                                "int b = 2;\n"
                                "int add_g() { return a + b; }\n"
                                "int main() { return add_g(); }\n",
                                false);
